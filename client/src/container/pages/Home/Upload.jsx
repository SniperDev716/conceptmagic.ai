...
import { Button, Col, Image, Layout, Row, Spin, Typography, Upload } from "antd";
  ...
  const handleNext = async () => {
    ...
    try {
      let res = await getImageDescription({
        fileLists: fileLists.filter(file => file.status == 'done').map(file => file.response?.path)
      });
      console.log(res);
      navigate(`/desc/${res.data.id}`);
    } catch (error) {
      console.log(error);
    }
    setLoading(false);
  ...
  return (
    <div className="text-center max-w-5xl w-screen mx-auto p-2">
      <Row gutter={[32, 32]} className="mt-6">
        <Col span={24}>
          <Title level={2}>Upload your idea images</Title>
        </Col>
        <Col span={24}>
          <div>
            <Dragger
            ...
            >
              ...
              <p className="ant-upload-text">
                Click or drag your image files to this area.
              </p>
